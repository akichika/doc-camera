...
- 🎥 **Camera & screen share** — pick any connected camera from a dropdown, or share a screen/window. Toggle cameras on/off with a checkbox without disconnecting the stream.
- 📷 **Dual camera** — when 2 or more cameras are connected, a second camera dropdown appears; each camera remembers its own zoom, pan, and keystone state for the session.
- 🔦 **Flashlight** — on mobile devices with a rear camera, a torch button appears next to the camera selector when the hardware supports it.
- 🪟 **Dual view** — show camera + screen (or camera + camera) side-by-side (left/right or top/bottom) with a draggable divider, **corner PiP** (8-position anchor grid, adjustable opacity, drag to reposition, drag handle to resize), or **semi-transparent overlay** (adjustable opacity) — with one-tap source swap.
...
- 🎥 カメラ選択（プルダウン）・画面共有。チェックボックスで映像の表示／非表示を接続したまま切り替え可能
- 📷 カメラが2台以上ある場合、カメラ2のプルダウンが表示される（ズーム・パン・台形補正などカメラごとに状態を維持）
- 🔦 スマホ・タブレット等の背面カメラ対応機種では、ライトボタンが自動表示
- 🪟 カメラ＋画面（またはカメラ＋カメラ）の左右／上下分割（境界線ドラッグ可）、**コーナーPiP**（8方向アンカー・透明度調整・ドラッグ移動＆リサイズ）、**半透明重畳**（透明度調整）、ソース入替
